        ...
        for userid in request.vars.keys():           #For each user selected..
            if userid.isdigit():
                ...
                for item in request.vars[userid]:
                    projectList.append(int(item))    #Add the checked projects to the user's project list
                    
                db(db.auth_user.id == int(userid)).update(projects=projectList)    #Update the database with the user's new project information
...
def deletefromproject():
    ...
    if table.accepts(request.vars): 
        for userid in request.vars.keys():                                    #For each user selected..
            if userid.isdigit():
                user = db(db.auth_user.id == int(userid)).select().first()    #Get the user
                projects = user.projects                                      #Get all the projects the user was associated with
                
                for item in request.vars[userid]:
                    projects.remove(int(item))                                #Remove the checked projects

                db(db.auth_user.id == int(userid)).update(projects=projects)  #Update the database with the user's new project information
